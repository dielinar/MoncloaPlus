...
fun GymScreen(
    ...
    LaunchedEffect(Unit) {
        resViewModel.fetchReservationsByDate(GYM_INDEX, currentDate)
    }

    Scaffold(
        floatingActionButton = {
            NewReservationButton(GYM_INDEX, resViewModel)
        },
        ...
        Column(modifier = Modifier.fillMaxSize().padding(innerPadding)) {
            ...
            Box(
                ...
                MyReservationsButton(
                    ...
                    onSelectedChange = { newValue ->
                        selected = newValue
                        if (newValue) resViewModel.fetchUserReservations(GYM_INDEX, currentDate)
                    }
                )
            }
            ...
            if (isLoading) {
                ...
                val reservationsToShow = if (selected) {
                    userReservations[GYM_INDEX]?.get(normalizedCurrentDate)
                } else {
