...
fun MusicStudioScreen(
    ...
    LaunchedEffect(Unit) {
        resViewModel.fetchReservationsByDate(MUSIC_STUDIO_INDEX, currentDate)
    }

    Scaffold(
        floatingActionButton = {
            NewReservationButton(MUSIC_STUDIO_INDEX, resViewModel)
        },
        ...
        Column(modifier = Modifier.fillMaxSize().padding(innerPadding)) {
            ...
            Box(
                ...
                MyReservationsButton(
                    ...
                    onSelectedChange = { newValue ->
                        selected = newValue
                        if (newValue) resViewModel.fetchUserReservations(MUSIC_STUDIO_INDEX, currentDate)
                    }
                )
            }
            ...
            if (isLoading) {
                ...
                val reservationsToShow = if (selected) {
                    userReservations[MUSIC_STUDIO_INDEX]?.get(normalizedCurrentDate)
                } else {
                    reservationsByDate[MUSIC_STUDIO_INDEX]?.get(normalizedCurrentDate)
                }
